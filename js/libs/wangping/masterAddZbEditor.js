	...
	render() {
		return (
			<div className="InputNumber" id={this.props.id || ''}>
				<input type="text" onInput={this._input.bind(this)} ref="scoreNum" value={this.state.number} />
	...
	sure() {
		...
		if (this.getTotal.call(this, 0) === 100 &&
			...
			if (this.state.flag) {
				...
				setTimeout(() => { this.setState({ flag: true }) }, 1000);
			}
	...
	render() {
		...
		return (
			<div className="editorBody" >
				...
				<p className="savetip">理论课和实训课填完后，点击【保存】</p>
				<div className="btnWarp">
					<button className="left" onClick={this.sure.bind(this)}>保存</button>
...
class ByCommon extends React.Component {
	...
	_add() {
		...
		if (!projectWarn && !descWarn) {
			...
			let newScore = this.getTotal() + this.inputNum.refs.scoreNum.value;
			if (newScore > 100) {
				Alert.open({
					alertTip: '通用课指标总分超过100！'
				});
			} else {
		...
		return (
			<div>
				...
				<div className="inputWarp">
					...
					<BluMUI_InputNumber id="commonInputScore" ref={(ref) => this.inputNum = ref} index={0} update={this._update.bind(this)} />
				</div>
				<span className="warn"></span>
				<div className="inputWarp">
					<span className="title must">评审标准</span>
					<textarea type="text" id="textArea" className="text" placeholder="最多不超过50个字" maxLength="50" ref={(input) => { this.textAreaInput = input }} onKeyDown={this.key}/>
					<span className="add" onClick={this._add.bind(this)}>（回车）添加</span>
				</div>
				...
				<div className="editorBody">
					...
					<div className="btnWarp">
						<button className="left" onClick={this._sure.bind(this)}>保存</button>
						<button className="right" onClick={this.props.off}>取消</button>
					</div>
				</div>
			</div>
...
class AddZbEditor extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			pcWarn: '',
			isEditor: this.props.isEditor || false
		}
	}
	off() {
