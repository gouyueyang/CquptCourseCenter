...
class List extends React.Component {
  ...
  option(type, data, data2, data3, eve) {
    ...
    switch (type) {
      case 'edit':
        window.location.href = `../systemManage/materCourseSort.html?fzx=${data2}&wppc=${data}&wppcId=${data3}&oldLocation=${window.location.href}`;
        break;
      case 'delete':
...
class BlueMUI_CreateOptions extends React.Component {
  constructor(props) {
    ...
    this.hand_serch = this.hand_serch.bind(this);
    ...
    if (this.state.subModule !== 'audit') {
      double_option = <div>
        <div id="out_serch">
          <span>课程名称：</span>
          <div id="search-box">
            <input type="text" id="jxtdss" ref="serchValue" onKeyDown={this.key}/>
            <span id="serch_btn" ref="serchBtn" onClick={this.hand_serch}>搜索</span>
          </div>
        </div>
      </div>;
    } else {
