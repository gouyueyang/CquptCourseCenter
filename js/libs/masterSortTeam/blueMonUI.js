...
class EditorList extends React.Component {
	...
	_search() {
		this.props.search(this.state.courseArea.trim(),this.state.evaluateName.trim(),this.state.dw.trim());
	}
	//回车搜索
    key(event) {
			...
			<div>
					...
					<div className="funcWarp">
						...
						< Radio select={masterType === 0 ? true : false} callback={this.selectMasterType.bind(this, 0)} />
						{
							...
							<div style={{ display: 'inline-block' }}>
								...
								<input type="text" className="searchInput" onInput={(e) => this.setState({courseArea : e.target.value})} onKeyDown={this.key}/>
							</div>
						}

						{
							...
							<div style={{ display: 'inline-block' }}>
								...
								<BluMUI_Drop  {...college} /> */}
								<span className="radioTitle" style={{ marginLeft: 6 }}>单位</span>
								<input type="text" className="searchInput" onInput={(e) => this.setState({dw : e.target.value})} onKeyDown={this.key}/>
							</div>
						}

						<span className="radioTitle" style={{ marginLeft: 6 }}>专家姓名</span>
						<input type="text" className="searchInput" onInput={(e) => this.setState({evaluateName : e.target.value})} onKeyDown={this.key}/>
				...
				<div className="ListWarp">
					<ul className="headerLine">
						...
						<li className="course">学科领域/单位</li>
