...
var BluMUI = require('../../libs/wangping/masterCourseSort.js'),
...
function exchangeSingle(type, index, that) {
	...
	addItems[0] && courseNos.push(addItems[0].courseId);
...
function search(College, CourseDepartment, CourseName) {
	curPage = 1;
	[curCollege = '', curCourseDepartment = '', curCourseName = ''] = [College, CourseDepartment, CourseName];
	getList({
		college: curCollege,
		courseDepartment: curCourseDepartment,
		courseName: curCourseName,
...
back.onclick = function () {
	let oldLocation = parseHash(window.location.href).oldLocation || "";
	if (oldLocation) {
