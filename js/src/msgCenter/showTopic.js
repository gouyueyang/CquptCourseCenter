...
if (hash.modelName === 'reply') {
	...
	getTeacherList().then(retTeacherList => {
		...
		getClassListFun(teacherList.jsList[0].SFRZH).then(retClassList => {
			...
			userType = User.id ==1 ?'游客':(classList.jxb ? "学生" : "教师");
			switch(userType){
				case '游客':jsSfrzh = teacherList.jsList[0].SFRZH;break;
				case '学生':jsSfrzh = classList.jxb[0].SFRZH;break;
				case '教师':
			...
			getClassListFun(jsSfrzh).then(retClassList => {
				classList = retClassList;
				getTopicListFun({ jxbh: classList.jxb ? classList.jxb[0].JXB : classList.allJxbList[0].JXB, pxtype: 'sjjx', sstype: 'htmc', sstj: '', page: 1, count: 5 }).then(retTopicList => {
